...
const getRemittancies = async() => {
    let query = ` SELECT
    CU.CUSTOMER_NAME, CU.ADDRESS, CO.COUNTRY_NAME, 
    REM.sending_amount, CH.AMOUNT, REM.receiving_amount, PAY.PAYMENT_TYPE, 
    ST.STATUS_TYPE, USR.FULLNAME, REM.transaction_date
    FROM REMITTANCE REM
        INNER JOIN CUSTOMERS CU ON REM.CUSTOMER_ID = CU.CUSTOMER_ID
        INNER JOIN ALLCOUNTRIES CO ON REM.COUNTRY_ID = CO.COUNTRY_ID
        INNER JOIN CHARGES CH ON REM.CHARGE_ID  = CH.CHARGE_ID
        INNER JOIN PAYMENTS PAY ON REM.payment_id = PAY.payment_id
        INNER JOIN STATUS ST ON REM.STATUS_ID = ST.STATUS_ID
        INNER JOIN USERS USR ON REM.USERID = USR.USERID`;
